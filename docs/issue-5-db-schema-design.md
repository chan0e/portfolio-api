...
| 중복방지한다(deduplicate) | 동일 범위 내 중복 레코드를 막는다 | `UNIQUE` 제약(예: `portfolio_id + title`) |
...
- 동등 레벨 중복 방지는 도메인 키 조합으로 제약한다.
  - 예: `UNIQUE(portfolio_id, anchor_id)`, `UNIQUE(portfolio_id, category)`, `UNIQUE(project_id, name)`
...
- 타입 전략: `BIGINT IDENTITY`, `VARCHAR`, `TEXT`, `BYTEA`, `SMALLINT`, `TIMESTAMPTZ`, `CHAR(64)` 사용.
...
- 핵심 제약
  - `UNIQUE(portfolio_id, company, position, start_date, end_date)`
  - `UNIQUE(portfolio_id, display_order)`
  - `CHECK(display_order >= 0)`
