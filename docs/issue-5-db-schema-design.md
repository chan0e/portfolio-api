...
### PK/FK 및 식별/비식별 관계 검토
- 대리키 PK 사용 여부: 적절
  - 모든 테이블이 `*_id BIGINT GENERATED BY DEFAULT AS IDENTITY` + `PRIMARY KEY` 패턴을 사용한다.
  - 루트와 하위 엔티티를 모두 대리키 기준으로 관리하므로 마이그레이션, 참조 변경, 확장 컬럼 추가에 유리하다.
- 1:1 관계의 FK + `UNIQUE` 구성: 적절
  - `profile.portfolio_id`, `contact.portfolio_id`, `project_link.project_id`는 FK이면서 `UNIQUE`로 강제되어 1:1 의미가 보존된다.
  - 공유 PK 방식보다 결합도가 낮아서 후속 변경과 롤백에 더 유연하다.
- 비식별 관계 사용 여부: 적절
  - `profile_bio`, `nav_item`, `skill_group`, `skill_item`, `project`, `project_stack`, `project_detail`, `experience`, `experience_achievement`는 모두 부모 FK를 자식 PK에 포함하지 않는 비식별 관계다.
  - 현재 포트폴리오/콘텐츠형 데이터 구조에서는 자식의 독립적 정렬, 수정, 이관 가능성을 남겨두는 쪽이 실무적으로 유리하다.
- 필요한 부분만 식별관계로 되어 있는지: 충족
  - 현재 스키마는 사실상 식별관계를 사용하지 않고, 필요한 강한 종속성은 `UNIQUE FK`로 해결한다.
  - `profile`, `contact`, `project_link`는 도메인상 강한 1:1 종속 후보지만, 공유 PK 대신 대리키 + `UNIQUE FK`를 택한 현재 설계도 충분히 타당하다.
- 잔여 리스크
  - `updated_at` 관리가 애플리케이션 계층으로 이동했으므로 쓰기 경로에서 일관되게 갱신하지 않으면 운영 데이터 품질 차이가 생길 수 있다.
  - 향후 `profile/contact`를 완전 종속 서브엔티티로 더 강하게 묶고 싶다면 공유 PK 방식 재검토 여지는 있다. 다만 현재 단계에서는 변경 비용 대비 이점이 크지 않다.
## 오픈 포인트 (확인 필요)
